...
- The URL comes from `runtimeConfig.public.pocketbaseUrl` (`NUXT_PUBLIC_POCKETBASE_URL`). Never hardcode one anywhere but `DEPLOYED_POCKETBASE_URL` at the top of `nuxt.config.ts`, which a Workers build (`WORKERS_CI`) forces into the variable and every other context ignores. `ssr: false` bakes the value in at **build** time, the build environment is a dashboard this repo cannot version, and an assets-only Worker holds no runtime variable at all — a build variable set to localhost there shipped an app dialling the phone it ran on, twice, presenting as a login that would not go through. Setting `runtimeConfig` instead does not work: a `NUXT_PUBLIC_*` variable overrides the config, so the deploy must overwrite the variable itself. Repoint a deploy by editing that line.
...
- Nothing in the layout may cover a screen's own header — its back control, its kebab, its search field. The offline notice sits in the page flow and is drawn in neutral card colours: a coloured card floating over the saturated header block read as something that had come loose, and on a box screen the block underneath is that box's own colour.
...
- The manifest link is declared in `app.head.link`, and `beforeinstallprompt` is captured in `plugins/installPrompt.client.ts` rather than in `<InstallPrompt>`. `ssr: false` means @vite-pwa's `<VitePwaManifest>` adds its link only after load, so the prerendered shell shipped without one and neither Chrome nor Firefox ever offered to install; and the event fires during that same first load, when `/login` (`layout: false`) has no `<InstallPrompt>` mounted to hear it. `tests/e2e/pwa.spec.ts` asserts the served HTML, not the live DOM. `<InstallPrompt>` owns its own fixed placement and is mounted by both the layout and `/login`: the plugin suppresses Chrome's mini-infobar in favour of it, so gating it behind an account left a signed-out visitor no way to install at all. The `maskable` icon exports are full-bleed — transparent corners let the launcher fill them with its own plate, which is the pale border around the installed icon; only the `any` exports keep the rounded tile.
- `app.head.viewport` carries `viewport-fit=cover`. Without it every `env(safe-area-inset-*)` in the app resolves to 0 — unnoticeable in a browser tab, wrong in the installed `display: standalone` window, where the page runs under the system bars.
...
- Navigation is one floating pill (`AppNav`), and a screen hides it with `definePageMeta({ nav: false })` — detail screens and forms own that space for their own actions. A page that hides it must supply its own bottom padding. `offlineBanner: false` does the same for the layout's offline notice, for a screen that carries its own.
